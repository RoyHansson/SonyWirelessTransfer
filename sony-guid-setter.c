...
static char sony_hostname[8] = "linux";
...
static int sony_guid_setter(libusb_device_handle *handle, uint8_t endpoint_in, uint8_t endpoint_out)
{
	...
	uint8_t lun;
	uint32_t expected_tag;
	size_t hostname_length, hostname_index;
	...
	hostname_length = strlen(sony_hostname);
	for (hostname_index = 0; hostname_index < hostname_length; hostname_index++) {
		cmd.name[hostname_index * 2] = (uint8_t)sony_hostname[hostname_index];
		cmd.name[hostname_index * 2 + 1] = 0;
	}

	printf("   using Sony hostname '%s'\n", sony_hostname);
	...
	// Send bulk
	CALL_CHECK(libusb_bulk_transfer(handle, endpoint_out, (unsigned char *)&cmd, sizeof(cmd), &size, 1000));
	printf("   sent %d bytes\n", size);
...
int main(int argc, char** argv)
{
			...
			if ( ((argv[j][0] == '-') || (argv[j][0] == '/'))
			  && (arglen >= 2) ) {
				...
				case 'l':
					...
					case 'n':
						if ((j+1 >= argc) || (argv[j+1][0] == '-') || (argv[j+1][0] == '/')) {
							printf("   Option -n requires a hostname (max 7 ASCII characters)\n");
							return 1;
						}
						if (strlen(argv[j+1]) > 7) {
							printf("   Option -n only supports hostnames up to 7 ASCII characters\n");
							return 1;
						}
						strncpy(sony_hostname, argv[++j], sizeof(sony_hostname) - 1);
						sony_hostname[sizeof(sony_hostname) - 1] = 0;
						break;
					default:
						show_help = true;
						break;
	...
	if ((show_help) || (argc == 1) || (argc > 9)) {
		printf("usage: %s [-h] [-d] [-g] [-n hostname] [-l lang] [vid:pid]\n", argv[0]);
		printf("   -h      : display usage\n");
		printf("   -d      : enable debug output\n");
		printf("   -g      : Sony Set GUID\n");
		printf("   -n host : Sony hostname to write over USB (max 7 ASCII characters)\n");
